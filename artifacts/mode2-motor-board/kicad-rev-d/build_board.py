...
uid = lambda s: str(uuid.uuid5(uuid.NAMESPACE_URL, 'mode2-motor-rev-d/' + s))
...
(ROOT/(NAME+'.kicad_sch')).write_text(f'''(kicad_sch(version 20250114)(generator "eeschema")(uuid {SCH_ID})(paper "A4")
 (title_block(title "模式二馬達控制板")(date "2026-09-09")(rev "D 原型")(company "STEAM"))
 (lib_symbols {embedded}) {''.join(sch)} (embedded_fonts no))''',encoding='utf-8')

# 雙面 24 x 24 mm，所有元件在正面；底面主要為接地平面。
...
for a,z in [((0,0),(24,0)),((24,0),(24,24)),((24,24),(0,24)),((0,24),(0,0))]:
...
for layer in [p.F_Cu,p.B_Cu]:
    ...
    for x,y in [(.35,.35),(23.65,.35),(23.65,23.65),(.35,23.65)]:outline.Append(mm(x),mm(y))
    b.Add(zone)
for x,y in [(1,9.5),(11,9.2),(21,9),(11,16.6),(17.5,15.5),(6.5,11.3),(22.7,18)]:via('GND',x,y)

for ref,(x,y) in {'U1':(6,13),'U2':(13.4,15),'C1':(6,9.3),'C2':(16,17.4),'C3':(16.65,5.7),'R1':(11.4,14.3),'S0':(5.1,.8),'Q1':(9.1,5.0),'R2':(3.7,8.0),'R3':(1.2,8.5),'P5':(16.7,18.6)}.items():
