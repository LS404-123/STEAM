...
KiCad 10 專案。PCB 尺寸 **24 × 24 mm**，雙面、1.6 mm FR-4、外層銅厚 1 oz；所有元件裝在正面。S0 改為薄型滑動開關，由 Q1 接通電源；電池、馬達使用 XH 插座；SW1、SW2 使用兩位螺絲端子。沒有 LED 和模式選擇跳線。
...
5. `fabrication/` 及 `mode2-gerbers-rev-d.zip` 提供打板資料；`BOM.csv` 是物料表。Gerber 已包含佈線和鋪銅。
...
板上共 **14 個元件**：2 顆 IC、1 顆 MOSFET、3 顆電阻、3 顆電容、1 個小滑動開關、2 個 XH 插座及 2 個螺絲端子。另需 2 組 XH 插頭線組；微動開關使用自帶導線。P5 是燒錄孔，可不裝排針。
...
| 插頭線組（板外） | XHP-2 兩組 | 接電池／馬達；SW1／SW2 不需要插頭 |
...
D 版將 SW1／SW2 改成螺絲端子；PCB 由 C 版 24 × 22 mm 改為 **24 × 24 mm**，增加的空間用於端子本體與旁邊元件的安裝公差。端子進線面朝板下邊，螺絲上方需預留螺絲批操作空間；XH 插座上方需預留插拔空間。規格依 [JST XH 原廠資料表](https://www.jst-mfg.com/product/pdf/eng/eXH.pdf) 及 [KEFA KF128-2.54 原廠圖面](https://datasheet.lcsc.com/datasheet/pdf/7193d1712e806c94a94291057681f262.pdf?productCode=C474920)。
...
- 3D 預覽使用 KiCad 通用元件模型；S0、U2、P2／P3 是依封裝尺寸製作的外形示意（端子螺絲及進線口為視覺示意）。3D 圖不作外殼精密干涉檢查依據。

## 檔案來源

SOIC、SOT-23、WSON、電阻、電容及 JST XH 插座焊盤沿用 KiCad 10 官方封裝庫；MOSFET 符號取自 KiCad Transistor_FET:Q_PMOS_GSD；S0 焊盤依 SHOU HAN 2024-12-14 圖面建立；P2／P3 焊盤依 KEFA 2021-03-13 圖面建立。KiCad 庫資產的授權資訊見 [KiCad Libraries License](https://www.kicad.org/libraries/license/)。廠方 PDF 與 STEP 原始資料保留原有權利聲明。
