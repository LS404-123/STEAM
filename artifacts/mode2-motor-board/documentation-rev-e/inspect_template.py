...
with ZipFile(ref) as z:
    inventory = {n: {'size': len(z.read(n)), 'sha256': sha256(z.read(n)).hexdigest()} for n in z.namelist()}
(root/'qa/template-inventory.json').write_text(json.dumps(inventory, indent=2), encoding='utf-8')
(root/'qa/artifact.md').write_text(f'''# 文件範本執行契約
來源：{ref.as_posix()}
SHA-256：{sha256(ref.read_bytes()).hexdigest()}
...
''', encoding='utf-8')
print('template contract and inventory ready')
