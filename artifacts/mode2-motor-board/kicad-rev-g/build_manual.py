"""從 G 版原生 KiCad 圖像及檢查結果輸出使用手冊；無 R/C 計算。"""
...
c.setTitle('模式二馬達控制板 G 版使用手冊｜40 × 40 mm')
...
def start(title,subtitle):
    ...
    para('MODE 2  /  G',LEFT+17*mm,H-16.5*mm,60*mm,9,GRAY)
    ...
    c.drawString(LEFT,11*mm,'G 版 · 2026-09-09 · 設計原型，尚未實機測試')
...
y-=14*mm
y=table(['比較','F 版','G 版（本版）'],[
    ['PCB 長 × 闊','28 × 28 mm','40 × 40 mm'],
    ['板面面積','784 mm²','1,600 mm²；約 2.04 倍'],
    ['電源／馬達主幹','0.60 mm','1.00 mm'],
    ['零件／操作','22 個正面零件','同樣 22 個；操作相同'],
],[54,54,66],y,9.5)
y=para('圖中紅／藍色是銅箔，包含接地鋪銅；白色是無銅空隙。新版端子較分開，標示較清楚；晶片的去耦電容仍靠近晶片。',LEFT,y)-4*mm
y=para('WSON 晶片短引出線為 0.20 mm，一般訊號線 0.25 mm；正反面以過孔相連，這仍是一塊板。走線轉彎已檢查，沒有 90° 轉彎；焊盤內接合及 T 形分支另計。',LEFT,y)-4*mm
para('加闊走線可減少同長度銅線的電阻，但本次也改變了路徑長度；不能因此宣稱整板耗電、溫升或電池壽命已有特定改善。',LEFT,y,WIDTH,10,GRAY)

y=start('05  裝配、檢查與檔案','製板及裝配請使用整套 G 版資料，不要混用舊版本的製板檔。')
y=sub('給裝配店的重點',y)
y=para('U1／U2：SOIC-14 的 SN74HC14DR／SN74HC74DR。U3：DRV8212DSGR，WSON-8，底部散熱焊盤必須焊接；適合交由貼片店回流焊。',LEFT,y)-3*mm
y=para('S0：C&K 1101M2S3CQE2，PCB 直插型、4.70mm 腳距。四組端子：KEFA KF128-2.54-2P，2.54mm 腳距。C4 正極朝板下方，D1 色帶端朝左。完整物料表見 BOM.csv。',LEFT,y)-3*mm
y=para('S0 使用大電流開關，沒有 Q1；本版沒有降低它的採購成本。替代開關必須先核對尺寸、腳位及 DC 電流額定。商品圖片不能保證庫存。',LEFT,y)-5*mm
y=table(['現象','先檢查'],[
...
para('原廠資料：<link href="https://www.ti.com/lit/ds/symlink/sn74hc14.pdf" color="#007F82">HC14</link> · <link href="https://www.ti.com/lit/ds/symlink/sn74hc74.pdf" color="#007F82">HC74</link> · <link href="https://www.ti.com/lit/ds/symlink/drv8212.pdf" color="#007F82">DRV8212</link> · <link href="https://www.ckswitches.com/media/1429/1000.pdf" color="#007F82">S0 圖面</link> · <link href="https://www.ti.com/lit/an/slva959b/slva959b.pdf" color="#007F82">馬達驅動佈局指南</link>',LEFT,y,WIDTH,9,GRAY)
c.save()
assert page==6
writer=PdfWriter()
writer.append(str(QA/'manual-pages.pdf'))
writer.append(str(OUT/'原理圖-G.pdf'))
writer.add_metadata({'/Title':'模式二馬達控制板 G 版使用手冊 - 40 × 40 mm','/Author':'模式二控制板專案'})
destination=OUT/'模式二控制板G版使用手冊.pdf'
with destination.open('wb') as stream:writer.write(stream)
reader=PdfReader(destination)
assert len(reader.pages)==7
content='\n'.join(p.extract_text() or '' for p in reader.pages)
for required in ('40 × 40','SN74HC14DR','SN74HC74DR','DRV8212DSGR','COM','NO','SW1','SW2','尚未實機測試'):
