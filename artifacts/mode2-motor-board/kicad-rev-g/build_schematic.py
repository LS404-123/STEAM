...
def put(ref, typ, x, y, nets, value=None, footprint='', prop=None, board=True):
    if board:
        name = ('SOIC14' if ref in ('U1','U2') else 'WSON8' if ref=='U3' else
                'CK1101' if ref=='S0' else 'KF128_2P' if ref.startswith('P') else
...
put('S0','SPDT',73,47,{'1':'BATT+','2':'VCC','3':'DISCH'},'1101M2S3CQE2',prop=(65,33))
...
note('S0：1–2 開；2–3 關。C&K 1101M2S3CQE2，銀接點 6A／28V DC，PCB 直插。',15,83,1.1)
note('不能沿用 E1 的 50mA MSK12C02。R6 在關機時放掉電源電容電荷；D1 加快復位。',15,88,1.1)
...
note('同名網絡標籤均相連；_N / NOT_ 表示反相信號。搭配 G 版 PCB；電路圖直角不代表銅線轉角。',15,288,1.05)
...
(ROOT/(NAME+'.kicad_sch')).write_text(f'(kicad_sch(version 20250114)(generator "eeschema")(uuid {SID})(paper "A3")(title_block(title "免韌體模式二原理圖")(date "2026-09-09")(rev "G 原型"))(lib_symbols {embedded})'+''.join(items)+'(embedded_fonts no))',encoding='utf-8')
