...
uid = lambda s: str(uuid.uuid5(uuid.NAMESPACE_URL, 'mode2-motor-rev-c/' + s))
...
PARTS = {
 ...
 'P1': ('電池 XH-2', 'XH2', {'1':'BATT+','2':'GND'}),
 'P2': ('SW1 PH-2 插座', 'PH2', {'1':'SW1','2':'GND'}),
 'P3': ('SW2 PH-2 插座', 'PH2', {'1':'SW2','2':'GND'}),
 'P4': ('馬達 XH-2', 'XH2', {'1':'OUT1','2':'OUT2'}),
 'P5': ('UPDI 燒錄', 'Prog3', {'1':'VBAT','2':'UPDI','3':'GND'}),
}
STOCK = {
 'XH2': 'Connector_JST.pretty/JST_XH_B2B-XH-A_1x02_P2.50mm_Vertical.kicad_mod',
...
for fpname,model in [('MSK12C02','MSK12C02-envelope.wrl'),('WSON8','WSON8-envelope.wrl')]:
...
(ROOT/(NAME+'.kicad_sch')).write_text(f'''(kicad_sch(version 20250114)(generator "eeschema")(uuid {SCH_ID})(paper "A4")
 (title_block(title "模式二馬達控制板")(date "2026-09-09")(rev "C 原型")(company "STEAM"))
 (lib_symbols {embedded}) {''.join(sch)} (embedded_fonts no))''',encoding='utf-8')

# 雙面 24 x 22 mm，所有元件在正面；底面主要為接地平面。
...
for a,z in [((0,0),(24,0)),((24,0),(24,22)),((24,22),(0,22)),((0,22),(0,0))]:
...
track('BATT+',[padpos('P1',1),(17,3.4),(17,2),(9.7,2),(9.7,7.7),(9.3,7.7)],.8,p.B_Cu);via('BATT+',9.3,7.7)
...
track('BATT+',[(.8,4.8),(.8,3.8),(7.35,3.8),(7.35,4.6)],.3,p.B_Cu)
...
track('VBAT',[(15.3,8.6),(15.3,16.4),(15.225,16.4)],.8,p.B_Cu);via('VBAT',15.225,16.4)
...
track('VBAT',[(15.225,16.4),(15.3,16.475),padpos('P5',1)],.4,p.B_Cu)
...
core_track('SW2',[(2,14.7),(1.3,15.4),(1.3,18.3),(9.6,18.3),(9.6,16.8)],.25,p.B_Cu)
...
core_track('OUT1',[(16,11.45),(16.2,11.45),(18.1,9.55),(21.1,9.55),(21.1,10)],.6)
core_track('OUT2',[(14.95,10.95),(15.5,10.95),(16.2,10.3)],.2);core_via('OUT2',16.2,10.3)
core_track('OUT2',[(16.2,10.3),(17,11.1),(17,12.5),(21.1,12.5)],.6,p.B_Cu)
...
for layer in [p.F_Cu,p.B_Cu]:
    ...
    for x,y in [(.35,.35),(23.65,.35),(23.65,21.65),(.35,21.65)]:outline.Append(mm(x),mm(y))
    b.Add(zone)
for x,y in [(1,9.5),(11,9.2),(21,9),(11,16.6),(17.5,15.5),(6.5,11.3),(22.7,18)]:via('GND',x,y)

for ref,(x,y) in {'U1':(5.9,16.35),'U2':(13.4,15),'C1':(6,9.3),'C2':(16,17.4),'C3':(16.65,5.7),'R1':(11.4,14.3),'S0':(5.1,.8),'Q1':(9.1,5.0),'R2':(3.7,8.0),'R3':(1.2,8.5),'P5':(16.7,18.6)}.items():
    fps[ref].Reference().SetPosition(v(x,y));fps[ref].Reference().SetTextAngle(p.EDA_ANGLE(0,p.DEGREES_T))
text('電池',16.6,2.8,.8,p.F_SilkS,90)
text('馬達',17,13.5,.8,p.F_SilkS,90)
text('2xAA',19,1.2,.8,p.B_SilkS)
text('關',3.8,2.9,.8,p.B_SilkS);text('開',6.1,2.9,.8,p.B_SilkS)
text('電池',21.1,7.4,.8,p.B_SilkS);text('+',19.2,3.4,.8,p.B_SilkS);text('-',19.2,5.9,.8,p.B_SilkS)
text('SW1',3.5,20.9,.85,p.B_SilkS);text('SW2',10.6,20.9,.85,p.B_SilkS)
text('馬達',21.1,16.4,.85,p.B_SilkS)
for n,mark in [('1','V'),('2','UPDI'),('3','G')]:
    x,_=padpos('P5',n)
    text(mark,x,18.4,.8,p.B_SilkS)
text('模式二 C',8,10,.85,p.B_SilkS)
b.BuildConnectivity()
