...
# 使用者指定商品圖：4.8mm 腳距、1.3×0.8mm 接腳；1.85mm 成品孔。
# 商品圖明示手量誤差，實物腳距及接點方向仍須樣品確認。
s='(footprint "SS12D10G5"(version 20241229)(generator "pcbnew")(layer "F.Cu")(attr through_hole)'
s+='(descr "SS12D10G5 straight pin; user drawing 12.7x6.7; pitch 4.8; drill 1.85; marked 2A 125VAC; DC load unverified")'
for prop,val,y,layer in [('Reference','REF**',-4.3,'F.SilkS'),('Value','SS12D10G5',4.3,'F.Fab')]:
    s+=f'(property "{prop}" "{val}"(at 0 {y})(layer "{layer}")(effects(font(size .8 .8)(thickness .12))))'
for n,x in [('1',-4.8),('2',0),('3',4.8)]:
...
for layer,dx,width in [('F.Fab',0,.1),('F.SilkS',.12,.12),('F.CrtYd',.3,.05)]:
    s+=f'(fp_rect(start {-6.35-dx} {-3.35-dx})(end {6.35+dx} {3.35+dx})(stroke(width {width})(type solid))(fill none)(layer "{layer}"))'
s+='(model "${KIPRJMOD}/models/SS12D10G5-envelope.wrl"(offset(xyz 0 0 0))(scale(xyz 1 1 1))(rotate(xyz 0 0 0))))'
(LIB/'SS12D10G5.kicad_mod').write_text(s,encoding='utf-8')
...
box(0,0,.8,12.7,6.7,1.6,'.08 .08 .08')
box(0,0,3.95,12.7,6.7,4.7,'.65 .67 .69')
box(-1.1,0,8.8,3.9,3.9,5,'.08 .08 .08')
for x in (-4.8,0,4.8): box(x,0,-3.45,1.3,.8,6.9,'.72 .72 .74')
(ROOT/'models/SS12D10G5-envelope.wrl').write_text('\n'.join(model),encoding='utf-8')
...
for layer in (p.F_SilkS,p.B_SilkS):
    text('MODE 2 / H',10,11,1,layer)
    text('ON',5.2,1,.85,layer);text('OFF',14.8,1,.85,layer)
