"""以本專案原理圖資料建立 28×28mm 正面裝配 PCB；封裝沿用已核對的 E1 庫。"""
...
for name in nc.values():
    n=p.NETINFO_ITEM(b,name);b.Add(n);nets[name]=n
positions={
 'S0':(8,4,0),'C4':(17.65,4,90),'P1':(24.6,5.3,90),
 'P4':(24.6,13.4,90),'P2':(2.5,24.5,0),'P3':(9,24.5,0),
 'U1':(4.5,13.8,0),'U2':(14.5,13.8,0),'U3':(21.7,19,180),
 'C1':(4.5,8.35,0),'C2':(14.5,8.35,0),'C3':(21.6,22,0),
 'R1':(3,19.7,0),'R2':(12,19.7,0),
 'R3':(9,12.6,90),'R4':(9,18.6,90),
 'C5':(9,9.6,90),'C6':(9,15.6,90),
 'R5':(15,24,90),'C7':(18,24,90),'D1':(16,20.5,0),
 'R6':(19.4,10.4,90),
}
fps={}
for part in parts:
    ...
    b.Add(f);fps[ref]=f
for a,z in [((0,0),(28,0)),((28,0),(28,28)),((28,28),(0,28)),((0,28),(0,0))]:
...
text('MODE 2 / F',8,6.5,.9)
text('2xAA',23,1.2,.8)
text('ON',3.3,1.0,.8);text('OFF',12.7,1.0,.8)
text('BAT',22,7.7,.8);text('+',26.7,5.3,.8);text('-',26.7,2.76,.8)
text('MOTOR',22.4,15.7,.8);text('M1',26.9,13.4,.7);text('M2',26.9,10.86,.7)
text('SW1',3.77,21.9,.8);text('SW2',10.27,21.9,.8)
for x in (2.5,9):text('NO',x,27,.7);text('COM',x+2.54,27,.65)
text('SW1',.5,24.5,.8,p.F_SilkS,90);text('SW2',7.0,24.5,.8,p.F_SilkS,90)
text('BAT',24,7.7,.7,p.F_SilkS);text('MOTOR',24,15.8,.7,p.F_SilkS)
text('F',26,25,1.2,p.F_SilkS)
b.BuildConnectivity()
