...
failed=[]
order=['M1','M2','VCC','BATT+','DISCH','DB1','DB2','CLK1','CLK2','RESET_N','RUN','REV','NOT_REV','POR_INV','POR_RC','SW1','SW2','GND']
for net in order:
    if net not in points:continue
    width=.6 if net in ('M1','M2','BATT+','VCC') else .25
    if net=='GND':width=.3
...
for layer in layers:
    ...
    for x,y in [(.35,.35),(27.65,.35),(27.65,27.65),(.35,27.65)]:poly.Append(mm(x),mm(y))
    b.Add(zone)
